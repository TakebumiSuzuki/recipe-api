...
* **`json` 型の場合**:
  保存されているテキストを、**クエリが走るそのつど JSON として先頭から構文解析（パース）し直します**（1万件のレコードがあれば、1万回パーサが動きます）。
  （テキストのまま保存されているため、目的のキーを探すには毎回パースして走査する必要があり、データ量が増えるほどCPU負荷が跳ね上がります。）
* **`jsonb` 型の場合**:
  保存時にあらかじめ構造化して「目次（キーと位置のオフセット）」が作られているため、パーサは動かず**目次を見て該当する場所へ直接ジャンプしてピンポイントで読むだけ**で済みます。
  （※1万件あれば「1万回目次を引く処理」自体は発生しますが、パース処理がないため1件ごとのコストが段違いに軽いです。）

```
1000 行を対象に ->> を実行したときの比較:

json 型:
  → 1000 行ぶん、重い JSON テキスト解析（パース）を毎回ゼロから実行する

jsonb 型:
  → 1000 行ぶん処理するのは同じだが、パースは不要。
     各行の目次（JEntry）を二分探索して該当位置の値を直接読むだけ（極めて軽量）
```
...
```sql
-- 「レシピテーブル の source カラムの全行から url の値を文字列として取得」
...
**`jsonb` はカラム全体に GIN（構造検索用インデックス）を貼れるが、`json` には貼れない。**

* **`json` 型**:
  特定のキーを抽出する「式インデックス（B-Tree）」であれば貼ることは可能（例: `CREATE INDEX ON recipes ((source->>'url'))`）。ただし、カラム全体に対するインデックスや、`@>`・`?` などの構造を対象とした GIN インデックスは貼れません。
* **`jsonb` 型**:
  式インデックスはもちろん、カラム全体に **GIN インデックス** を貼ることで、任意のキーの存在確認（`?`）や構造の包含検索（`@>`）を高速に行えます。
...
| キー取り出し `->` `->>` | できる | できる |
| 包含・キー存在 `@>` `?` | **できない** | できる |
| インデックス | **式インデックスのみ** | **GIN も式インデックスも可** |
...
SQLAlchemy で JSON を扱う型は、大きく分けて **汎用型** と **PostgreSQL専用（方言）型** があります。

```python
from sqlalchemy import JSON                      # 汎用型（PostgreSQL では json 型になる）
from sqlalchemy.dialects.postgresql import JSONB  # PostgreSQL 専用（jsonb 型になる）
# ※ dialects.postgresql には専用の JSON 型（postgresql.JSON）もあります
```

汎用 `JSON` は「どのDBでも動く型」で、PostgreSQL ではネイティブの `json` 型に変換されます。
そのため、**汎用型のままでは `jsonb` にはなりません**。
`jsonb` を使いたい場合は、PostgreSQL 専用の引き出しである `sqlalchemy.dialects.postgresql` から `JSONB` をインポートして指定する必要があります。
...
`.astext`（`->>` 演算子を生成するプロパティ）は、PostgreSQL 専用の型（`postgresql.JSON` および `postgresql.JSONB`）にのみ用意されています。汎用 `sqlalchemy.JSON` では使えません（汎用型では `.as_string()` などを利用します）。
...
| 使い方 | 例 | 索引 |
|---|---|---|
| 保存して丸ごと取り出すだけ | 詳細画面に出典を表示する。行は `id` で引く | **不要**（主キー等で行を特定して丸ごと取り出すだけなら、JSON内部のインデックスは不要） |
...
- **`json` 型でもキーの取り出しはできる。** 「`json` は検索できない」は誤り。
  `->` `->>` `#>` `#>>` は両方で使える。差は「毎回パースするので遅い」「GIN インデックスが貼れない（特定キーの式インデックスは貼れる）」「`@>` や `?` が使えない」の3点。
- **JSONB の B は Binary（分解されたバイナリ形式: decomposed binary format）** のこと。
  単なる文字列のバイナリ化ではなく、保存時にパースして「キーの目次」と「値」に構造化されたバイナリレイアウトで保持される。だから空白もキー順も保持されない。
